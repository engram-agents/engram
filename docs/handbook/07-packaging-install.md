...
- **tests.yml** — pytest matrix 3.11/3.12 over tests/ + forum/tests/ (ENGRAM_NO_EMBEDDINGS=1). Runs on main push/PR + dispatch. *(Dispatcher note: forum's tests ARE in CI even though forum isn't in the manifest — test coverage and ship coverage diverge, the L5 paradox.)*
